...
// 수퍼 클래스에서 추상적으로 표현하고, 서브 클래스에서 구체적인 내용을 결정하는 설계 방식
// 즉 수퍼 클래스에서 전체적인 논리 흐름(build() 메서드)를 정의하고
// 서브 클래스에서 각 흐름에 따라 구체적인 동작(ex: startEffect(), endEffect()을 정의한다.

public abstract class Building {
  // 다음과 같이 일반 메서드에서
  // 전체적인 논리 흐름(명령어 처리의 흐름)을 정의한다.
  // 서브 클래스들은 이 메서드를 그대로 상속 받기 때문에 재정의하지 않는다.
